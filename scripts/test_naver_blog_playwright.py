...
async def cmd_post() -> None:
    """Test posting with structured content (headings, quotes, images)."""
    from app.services.blog_image_generator import generate_structured_content
    from app.services.naver_blog_playwright import NaverBlogPlaywright
    ...
    title = "ContentFlow SmartEditor Test"
    raw_text = (
        ...
        "This is a structured content test post with headings, "
        "images, quotes, and dividers generated by Claude AI. "
    ...
    # Generate structured content via Claude + pollinations.ai
    print("Generating structured content via Claude...")
    blocks = await generate_structured_content(title, raw_text)
    if not blocks:
        print("[WARN] Structured content generation failed, using plain text fallback")
        blocks = [
            {"type": "paragraph", "text": raw_text},
        ]

    print(f"Generated {len(blocks)} content blocks:")
    for b in blocks:
        btype = b.get("type", "?")
        preview = b.get("text", b.get("url", ""))[:40]
    ...
    result = await client.post(
        title=title,
        content=blocks,
        tags=["ContentFlow", "SmartEditor", "AI", "test"],
    )
